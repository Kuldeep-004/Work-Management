...
router.get("/user/:userId", protect, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .select("firstName lastName email role team photo hourlyRate")
      .populate("team", "name");
    if (!user) {
    ...
    timesheets.forEach((timesheet) => {
      ...
      timesheet.entries.forEach((entry) => {
        ...
        // Categorize time
        if (
          entry.manualTaskName === "Permission" ||
          ...
          entry.manualTaskName ===
            "INFRASTRUCTURE ISSUES & DISCUSSION WITH VIVEK SIR" ||
          entry.task === "infrastructure-issues"
        ) {
          dailyInfrastructure += minutes;
          analytics.summary.infrastructureHours += minutes;
          analytics.summary.totalWorkingHours += minutes;
        } else if (
          entry.manualTaskName === "DISCUSSION WITH VIVEK SIR" ||
          entry.task === "discussion-with-vivek"
        ) {
    ...
    analytics.dailyBreakdown.forEach((day) => {
      ...
      if (!weeklyData[weekKey])
        weeklyData[weekKey] = { totalHours: 0, days: 0 };
    ...
    analytics.monthlyTrend = Object.entries(monthlyData).map(
      ([month, data]) => ({
        month,
        totalHours: data.totalHours,
        averageDaily: data.totalHours / data.days,
    ...
    analytics.summary.totalSalary =
      totalWorkingHoursInHours * (user.hourlyRate || 0);
    ...
    if (daysWithTimesheets > 0) {
      analytics.productivity.averageDailyHours =
        analytics.summary.totalWorkingHours / daysWithTimesheets;
    }
