...
router.post('/templates/:templateId/verify', protect, async (req, res) => {
  try {
    ...
    if (action === 'approve') {
      ...
      // IMPORTANT: Reset automation run status when a new template is approved
      // This allows the automation to run again even if it already ran this month/period
      if (['dayOfMonth', 'quarterly', 'halfYearly', 'yearly'].includes(automation.triggerType)) {
        automation.lastRunDate = undefined;
        automation.lastRunMonth = undefined;
        automation.lastRunYear = undefined;
...
// Get automation execution status and next run times (admin only)
...
router.post('/:id/force-run', protect, async (req, res) => {
  try {
    ...
    // Log forced automation run
    await ActivityLogger.logSystemActivity(
      req.user._id,
      'automation_forced_run',
      automation._id,
      `Forced automation "${automation.name}" to run`,
      null,
      { automationName: automation.name, result },
      req
    );
...
// Reset automation run status (admin only, for testing)
router.post('/:id/reset-status', protect, async (req, res) => {
  try {
    // Only admins can reset automation status
    if (req.user.role !== 'Admin') {
      return res.status(403).json({ message: 'Not authorized, admin access required' });
    ...
    await ActivityLogger.logSystemActivity(
      ...
      'automation_status_reset',
      automation._id,
      `Reset run status for automation "${automation.name}"`,
      null,
      { automationName: automation.name },
      req
    );
    ...
    res.json({
      message: `Reset run status for automation "${automation.name}"`,
      automation
    });
  } catch (err) {
    console.error('Error resetting automation status:', err);
    res.status(500).json({ message: 'Failed to reset automation status', error: err.message });
  }
});
