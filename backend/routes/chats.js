...
router.get("/", protect, async (req, res) => {
  try {
    ...
    const chats = await Chat.find({
      ...
      .select(
        "type name description participants avatar lastMessage lastActivity unreadCounts settings"
      )
      .sort({ lastActivity: -1 })
      .limit(limit)
      .skip(skip)
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        .status(403)
        .json({
          message: "Access denied. Only system admins can create groups.",
        });
    }
    ...
    if (!name || !participantIds || participantIds.length < 1) {
      return res
        .status(400)
        .json({ message: "Group name and at least one participant required" });
    }
...
router.put("/:chatId", protect, async (req, res) => {
  try {
    ...
    if (!chat) {
      return res
        .status(404)
        .json({ message: "Group not found or insufficient permissions" });
    }
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        ...
        .json({
          message:
            "Access denied. Only system admins can add participants to groups.",
        });
    }
    ...
    chat.participants.push(
      ...newParticipantIds.map((id) => ({ user: id, role: "member" }))
    );
    await chat.save();
    ...
    try {
      ...
      if (req.user.role !== "Admin") {
        return res
          ...
          .json({
            message:
              "Access denied. Only system admins can remove participants from groups.",
          });
      }
      ...
      if (otherAdmins.length > 0) {
        ...
        if (otherMembers.length > 0) {
          ...
          chat.participants.find(
            (p) => p.user.toString() === otherMembers[0].user.toString()
          ).role = "admin";
        } else {
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        .status(403)
        .json({
          message: "Access denied. Only system admins can update group names.",
        });
    }
    ...
    try {
      ...
      if (req.user.role !== "Admin") {
        return res
          ...
          .json({
            message:
              "Access denied. Only system admins can update group avatars.",
          });
      }
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        ...
        .json({
          message:
            "Access denied. Only system admins can add members to groups.",
        });
    }
    ...
    // Add new participants
    chat.participants.push(
      ...newUserIds.map((id) => ({ user: id, role: "member" }))
    );
    await chat.save();
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        ...
        .json({
          message:
            "Access denied. Only system admins can remove members from groups.",
        });
    }
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        ...
        .json({
          message:
            "Access denied. Only system admins can manage group admin status.",
        });
    }
...
// Get all users for chat creation (exclude deleted users)
  ...
  try {
    ...
    if (req.user.role !== "Admin") {
      return res
        .status(403)
        .json({
          message: "Access denied. Only system admins can delete groups.",
        });
    }
