...
router.get("/my-tasks", protect, async (req, res) => {
  try {
    ...
    if (timesheetView === "team" && currentUser?.team) {
      // Get all users in the same team (only approved users)
  ...
  try {
    ...
    if (taskId && taskId.trim() !== "") {
      if (taskId === "other") {
        ...
        taskField = null;
        manualTaskNameField =
          "INFRASTRUCTURE ISSUES & DISCUSSION WITH VIVEK SIR";
...
router.delete("/entry/:entryId", protect, async (req, res) => {
  try {
    ...
    await ActivityLogger.logSystemActivity(
      ...
      timesheet._id,
      `Deleted timesheet entry${entryToDelete?.task ? " for task" : ""} "${
        entryToDelete?.manualTaskName || "Task entry"
      }"`,
      {
...
router.get("/subordinates", protect, async (req, res) => {
  try {
    ...
    if (isTeamHead) {
      ...
      if (req.user.team) {
        // Exclude Admin and Team Head roles
        subordinates = await User.find({
          team: req.user.team,
          isEmailVerified: true,
          status: "approved",
          role: { $nin: ["Admin", "Team Head"] },
        }).select("_id firstName lastName email role team photo");
      } else {
...
router.get("/subordinate/:userId/:date", protect, async (req, res) => {
  try {
    ...
    if (!timesheet) {
      return res
        .status(404)
        .json({ message: "Timesheet not found for this date" });
    }
      ...
      ts.entries.forEach((entry) => {
        if (entry.task && entry.task._id) {
          ...
          if (!taskUserHours[manualTaskKey][userId]) {
            taskUserHours[manualTaskKey][userId] = {
              userId,
              userName,
              totalMinutes: 0,
              isManualTask: true,
          ...
          taskUserHours[manualTaskKey][userId].totalMinutes +=
            getMinutesBetween(entry.startTime, entry.endTime);
        }
      });
    });
    ...
    Object.entries(taskUserHours).forEach(([taskId, users]) => {
      Object.values(users).forEach((userObj) => {
        result.push({
          ...
          totalHours:
            Math.round(((userObj.totalMinutes || 0) / 60) * 100) / 100, // 2 decimals
...
router.get("/user-hours", protect, async (req, res) => {
  try {
    ...
    const result = Object.values(userHours).map((user) => ({
      ...
      manualTaskHours:
        Math.round(((user.manualTaskMinutes || 0) / 60) * 100) / 100,
      regularTaskHours:
        Math.round(((user.regularTaskMinutes || 0) / 60) * 100) / 100,
    }));
...
router.get("/task-costs", protect, async (req, res) => {
  try {
    ...
    const timesheets = await Timesheet.find({})
      .populate("user", "firstName lastName hourlyRate")
      .populate("entries.task", "title");
    ...
    timesheets.forEach((ts) => {
      ...
      ts.entries.forEach((entry) => {
        if (entry.task && user) {
          ...
          if (!taskUserMinutes[taskId][userId])
            taskUserMinutes[taskId][userId] = 0;
          // Calculate minutes
          ...
          const userId = user._id?.toString();
          if (!userId) return;
          if (!taskUserMinutes[manualTaskKey])
            taskUserMinutes[manualTaskKey] = {};
          if (!taskUserMinutes[manualTaskKey][userId])
            taskUserMinutes[manualTaskKey][userId] = 0;
          // Calculate minutes
          const [sh, sm] = entry.startTime.split(":").map(Number);
          const [eh, em] = entry.endTime.split(":").map(Number);
          let startM = sh * 60 + sm;
    ...
    for (const task of tasks) {
      ...
      const totalCost =
        ...
        (thirdVerifier?.cost || 0) +
        (fourthVerifier?.cost || 0) +
        (fifthVerifier?.cost || 0) +
        guidesCost;
      result.push({
        taskId: task._id,
        title: task.title,
...
router.get("/task-costs/billed", protect, async (req, res) => {
  try {
    ...
    console.error("Error aggregating billed task costs:", error.stack || error);
    res.status(500).json({ message: "Server error" });
  }
});
...
router.get("/task-costs/billed/user/:userId", protect, async (req, res) => {
  try {
    ...
    // Build task filter - tasks where the user is assignedTo only
    let taskFilter = {
      billed: false,
      status: { $ne: "completed" },
      assignedTo: userId,
    ...
    timesheets.forEach((ts) => {
      ...
      ts.entries.forEach((entry) => {
        if (entry.task && user) {
          ...
          if (!taskId || !userIdStr) return;
          if (!taskUserMinutes[taskId]) taskUserMinutes[taskId] = {};
          if (!taskUserMinutes[taskId][userIdStr])
            taskUserMinutes[taskId][userIdStr] = 0;
          ...
          let endM = eh * 60 + em;
          if (endM < startM) endM += 24 * 60;
          taskUserMinutes[taskId][userIdStr] += endM - startM;
        }
      });
    });
    ...
    for (const task of tasks) {
      ...
      function getUserCost(user) {
        if (!user) return null;
        const userIdStr = user._id?.toString();
        const totalMinutes = taskUserMinutes[taskId]?.[userIdStr] || 0;
        const hours = Math.round((totalMinutes / 60) * 100) / 100;
        const cost = hours * (user.hourlyRate || 0);
        return {
          name: user.firstName + " " + user.lastName,
...
router.get("/task-costs/unbilled/user/:userId", protect, async (req, res) => {
  try {
    ...
    if (search) {
      taskFilter.title = { $regex: search.toLowerCase(), $options: "i" };
    }
...
router.get("/task/:taskId/timeslots", protect, async (req, res) => {
  try {
    ...
    const timesheets = await Timesheet.find({
      ...
      .populate("user", "firstName lastName role hourlyRate")
      .populate("entries.task", "title");
...
router.patch("/entry/:entryId", protect, async (req, res) => {
  try {
    const { entryId } = req.params;
    const { taskId, manualTaskName, workDescription, startTime, endTime } =
      req.body;
...
router.post("/submit", protect, async (req, res) => {
  try {
    ...
    if (!timesheet) {
      return res
        .status(404)
        .json({ message: "No timesheet found for the specified date." });
    }
...
router.patch("/entry/:entryId/approve", protect, async (req, res) => {
  try {
    ...
    const taskName = entry.task
      ? entry.task.title
      : entry.manualTaskName || "Manual Task";
...
router.patch("/:timesheetId/return", protect, async (req, res) => {
  try {
    ...
    await ActivityLogger.log({
      ...
      description: `Returned timesheet for editing: ${
...
router.patch("/:timesheetId/accept-all", protect, async (req, res) => {
  try {
    ...
    await ActivityLogger.log({
      ...
      entity: "Timesheet",
      entityId: timesheetId,
      description: `Accepted all ${
        entriesToAccept.length
      } pending/rejected entries for: ${timesheet.user.firstName} ${
        timesheet.user.lastName
      } (${timesheet.date.toISOString().split("T")[0]})`,
      metadata: {
        targetUserId: timesheet.user._id,
