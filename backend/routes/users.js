...
router.get("/", protect, async (req, res) => {
  try {
    ...
    // Otherwise, exclude rejected, pending, and deleted users by default
    else if (includeRejected !== "true") {
      query.status = "approved";
    } else {
      // Even with includeRejected, never show deleted users
  ...
  try {
    ...
    if (status === "approved") {
      if (!role) {
        return res
          .status(400)
          .json({ message: "Role is required for approval" });
      }

      // Validate role
      if (!["Fresher", "Senior", "Team Head"].includes(role)) {
        // swapped
      ...
      if (role === "Senior") {
        ...
        if (!team) {
          return res
            .status(400)
            .json({ message: "Team is required for non-Senior roles" });
        }
...
router.delete("/:userId", protect, async (req, res) => {
  try {
    ...
    await ActivityLogger.logUserActivity(
      ...
      "user_deleted",
      user._id,
      `Soft deleted user ${user.firstName} ${user.lastName} (${user.email})`,
      { status: user.status },
      { status: "deleted" },
      req,
      { userEmail: user.email },
...
// Admin: Get all users (except self) with hourly rates (exclude deleted users)
...
// Get all users except the current user (exclude deleted users)
...
router.post("/user-tab-state/:tabKey", protect, async (req, res) => {
  try {
    ...
    if (
      existing &&
      existing.state &&
      Array.isArray(existing.state.tabs) &&
...
router.patch("/tabstate/taskOrder", protect, async (req, res) => {
  try {
    ...
    if (!tabKey || !Array.isArray(order)) {
      return res
        .status(400)
        .json({ message: "tabKey and order array are required" });
    }
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to update task order" });
  }
});
...
router.patch("/tabstate/columnOrder", protect, async (req, res) => {
  try {
    ...
    if (!tabKey || !Array.isArray(columnOrder) || !tabId) {
      return res
        .status(400)
        .json({ message: "tabKey, tabId, and columnOrder array are required" });
    }
    const userId = req.user.id;
    let userTabState = await UserTabState.findOne({ user: userId, tabKey });
    if (!userTabState) {
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to update column order" });
  }
});
...
router.get("/tabstate/columnOrder", protect, async (req, res) => {
  try {
    ...
    if (
      userTabState?.state?.tabs &&
      Array.isArray(userTabState.state.tabs) &&
      userTabState.state.tabs.length > 0
    ) {
      let tabObj;
      if (tabId) {
        tabObj = userTabState.state.tabs.find((t) => t.id == tabId);
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to get column order" });
  }
});
...
router.get("/tabstate/taskOrder", protect, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to get task order" });
  }
});
...
router.patch("/tabstate/groupOrder", protect, async (req, res) => {
  try {
    ...
    if (!tabKey || !Array.isArray(groupOrder)) {
      return res
        .status(400)
        .json({ message: "tabKey and groupOrder array are required" });
    }
    const userId = req.user.id;
    let userTabState = await UserTabState.findOne({ user: userId, tabKey });
    if (!userTabState) {
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to update group order" });
  }
});
...
router.get("/tabstate/groupOrder", protect, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: err.message || "Failed to get group order" });
  }
});
