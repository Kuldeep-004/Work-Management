...
const FilterPopup = ({
  ...
  const baseFilterableColumns = [
    ...
    },
    {
      value: "priority",
      label: "Priority",
      options: priorities.map((p) => p.name),
    },
    {
      value: "assignedTo",
      label: "Assigned To",
  ...
  const handleAddFilter = () => {
    setEditingFilters([
      ...editingFilters,
      { column: "title", operator: "contains", value: "", saved: false },
    ]);
  };
  ...
  const handleFilterChange = (index, field, value) => {
    ...
    // Reset value when operator changes to/from between
    if (field === "operator") {
      if (value === "between" && typeof newFilters[index].value !== "object") {
        newFilters[index].value = { startDate: "", endDate: "" };
      } else if (
        newFilters[index].operator === "between" &&
        value !== "between"
      ) {
        newFilters[index].value = "";
      }
  ...
  const renderValueInput = (filter, index) => {
    ...
    // For date columns with 'between' operator, show two date inputs
    if (selectedColumn.type === "date" && filter.operator === "between") {
      const dateValue =
    ...
    if (
      ...
      filter.operator === "does_not_contain"
    ) {
      return (
        <input
          type="text"
          ...
          >
            {selectedColumn.options.map((opt) => {
              ...
              return (
                ...
                >
                  <span
                    className={`inline-block w-3 h-3 mr-2 rounded-sm border ${
                      isSelected
        ...
        {editingFilters.map((filter, index) => (
          ...
          >
            ...
            <div className="flex justify-end sm:justify-start gap-2">
              {filter.saved ? (
                <span className="text-sm font-medium text-green-600 w-20 text-center">
                  Saved
                </span>
              ) : (
                <button
                  onClick={() => handleSaveFilter(index)}
                  className="w-20 p-1 rounded-md bg-blue-500 text-white text-sm hover:bg-blue-600"
                >
                  Save
                </button>
              )}
