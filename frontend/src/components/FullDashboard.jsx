...
const FullDashboard = () => {
  ...
  const [showBulkPriorityDropdown, setShowBulkPriorityDropdown] =
    useState(false);
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/api/automations/${selectedAutomation._id}/tasks`,
          {
  ...
  const activeTabObj = useMemo(() => {
    return (
      ...
      tabs[0] ||
      DEFAULT_TAB(customColumnsLoaded ? customColumns : [])
    );
      ...
      setTabs(
        tabs.map((tab) => {
          ...
          if (patch.visibleColumns) {
            ...
            const newOrder = currentOrder.filter((colId) =>
              patch.visibleColumns.includes(colId),
            );
    ...
    (async () => {
      try {
        ...
        if (
          isMounted &&
          tabStates &&
          Array.isArray(tabStates.tabs) &&
          tabStates.tabs.length > 0
        ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !bulkStatusDropdownRef.current.contains(event.target)
      ) {
      ...
      if (
        bulkPriorityDropdownRef.current &&
        !bulkPriorityDropdownRef.current.contains(event.target)
      ) {
  ...
  const getFilteredAndSortedTasks = useCallback(
      ...
      let filteredTasks = tasks.filter((task) => {
        ...
        for (let i = 0; i < filters.length; i++) {
          ...
          if (operator === "any_of" && Array.isArray(value)) {
            filterResult =
              value.includes(String(taskValue)) ||
              value.includes(taskValue?._id);
    ...
    [
      priorities,
      activeTabObj.filters,
      activeTabObj.searchTerm,
      activeTabObj.sortBy,
      activeTabObj.sortOrder,
    ],
  );
    ...
    async (updatedOrCreated) => {
      ...
      if (isUpdate) {
        ...
        try {
          ...
          const res = await fetch(
            `${API_BASE_URL}/api/tasks/${updatedOrCreated._id}`,
            {
              headers: { Authorization: `Bearer ${user.token}` },
            },
          );
  ...
  const handleTemplateSubmit = async (updatedTemplate) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/automations/${selectedAutomation._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleTaskSelect = useCallback(
    (taskId, isSelected) => {
      setSelectedTasks((prev) => {
        ...
        const currentTasks = getFilteredAndSortedTasks(
          tasks,
          activeTabObj.filters,
        );
        setIsAllSelected(
          newSelected.length === currentTasks.length && currentTasks.length > 0,
        );
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (failedDeletes.length > 0) {
        toast.error(
          `Failed to delete ${failedDeletes.length} task${failedDeletes.length > 1 ? "s" : ""}`,
        );
      } else {
        toast.success(
          `Successfully deleted ${selectedTasks.length} task${selectedTasks.length > 1 ? "s" : ""}`,
        );
      }
  ...
  const handleBulkStatusUpdate = async (newStatus) => {
    ...
    if (newStatus === "completed") {
      ...
      const tasksWithoutSelfVerification = selectedTaskObjects.filter(
        (task) => !task.selfVerification,
      );

      if (tasksWithoutSelfVerification.length > 0) {
        toast.error(
          `Cannot set status to Completed. ${tasksWithoutSelfVerification.length} task${tasksWithoutSelfVerification.length > 1 ? "s" : ""} do not have self verification checked.`,
        );
        return;
      }
    }
    ...
    try {
      ...
      if (failedUpdates.length > 0) {
        toast.error(
          `Failed to update ${failedUpdates.length} task${failedUpdates.length > 1 ? "s" : ""}`,
        );
      } else {
        // Format status name for display
        const statusDisplay = taskStatuses.find(
        ...
        toast.success(
          `Successfully updated ${selectedTasks.length} task${selectedTasks.length > 1 ? "s" : ""} to ${formattedStatus}`,
        );
  ...
  const handleBulkPriorityUpdate = async (newPriority) => {
    ...
    try {
      ...
      if (failedUpdates.length > 0) {
        ...
        const priorityDisplay = priorities.find(
        ...
        toast.success(
          `Successfully updated ${selectedTasks.length} task${selectedTasks.length > 1 ? "s" : ""} to ${formattedPriority} priority`,
        );
        ...
        t._id === selectedTask._id
          ? {
              ...
              files: [
                ...(t.files || []),
                ...files.filter(
                  (uf) => !(t.files || []).some((f) => f._id === uf._id),
                ),
              ],
            }
          : t,
  ...
  const handleCommentSubmit = async (comment) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/tasks/${selectedTask._id}/comments`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDownloadPDF = (
    ...
    const allAvailableColumns = [
      ...ALL_COLUMNS.map((col) => {
        ...
        if (col.id === "description" || col.id === "status")
          return { ...col, defaultWidth: 140 };
        return col;
      }),
    ...
    if (groupBy && groupBy !== "") {
      ...
      Object.entries(groupedTasks).forEach(
        ([groupKey, groupTasks], groupIndex) => {
  ...
  const handleDownloadExcel = (
    ...
    try {
      ...
      XLSX.writeFile(
        wb,
        `tasks_${activeTabObj?.activeTab || "dashboard"}.xlsx`,
      );
  ...
  const handleITRDownloadPDF = (
    ...
    const itrData = itrTasks.map((task, index) => {
      ...
      if (assignedUser && assignedUser.team) {
        ...
        const assignedUserTeamId =
          typeof assignedUser.team === "object"
        ...
        teamHeadNames = teamHeadsOrAdmins
          .map((th) => `${th.firstName} ${th.lastName}`)
          .join(", ");
      }
      ...
      return {
        ...
        allotee: task.assignedTo
          ? `${task.assignedTo.firstName} ${task.assignedTo.lastName}`
          : "",
        draftFinancialsAndComputationPreparation:
          task.itrProgress?.draftFinancialsAndComputationPreparation === true
            ? "T"
            : task.itrProgress?.draftFinancialsAndComputationPreparation ===
                false
              ? ""
              : "",
        accountantVerification:
          task.itrProgress?.accountantVerification === true
            ? "T"
            : task.itrProgress?.accountantVerification === false
              ? ""
              : "",
        vivekSirVerification:
          task.itrProgress?.vivekSirVerification === true
            ? "T"
            : task.itrProgress?.vivekSirVerification === false
              ? ""
              : "",
        girijaVerification:
          task.itrProgress?.girijaVerification === true
            ? "T"
            : task.itrProgress?.girijaVerification === false
              ? ""
              : "",
        firstVerification: firstVerifier,
        secondVerification: secondVerifier,
        hariSirVerification:
          task.itrProgress?.hariSirVerification === true
            ? "T"
            : task.itrProgress?.hariSirVerification === false
              ? ""
              : "",
        issuedForPartnerProprietorVerification:
          task.itrProgress?.issuedForPartnerProprietorVerification === true
            ? "T"
            : task.itrProgress?.issuedForPartnerProprietorVerification === false
              ? ""
              : "",
        challanPreparation:
          task.itrProgress?.challanPreparation === true
            ? "T"
            : task.itrProgress?.challanPreparation === false
              ? ""
              : "",
        itrFiledOn: task.itrProgress?.itrFiledOn
          ? formatDate(task.itrProgress.itrFiledOn)
          : "",
        billPreparation:
          task.itrProgress?.billPreparation === true
            ? "T"
            : task.itrProgress?.billPreparation === false
              ? ""
              : "",
      };
    });
    ...
    const columnsToShow = selectedColumns
      .map((colId) => {
        switch (colId) {
          case "no":
            return { dataKey: "no", header: "No" };
          case "dataReceivedOn":
            return { dataKey: "dataReceivedOn", header: "Data Received on" };
          case "nameOfAssessee":
            return {
              dataKey: "nameOfAssessee",
              header: "Name of the Assessee",
            };
          case "teamHead":
            return { dataKey: "teamHead", header: "Team Head" };
          case "allotee":
            return { dataKey: "allotee", header: "Allotee" };
          case "draftFinancialsAndComputationPreparation":
            return {
              dataKey: "draftFinancialsAndComputationPreparation",
              header: "Draft Financials and Computation Preparation",
            };
          case "accountantVerification":
            return {
              dataKey: "accountantVerification",
              header: "Accountant Verification",
            };
          case "vivekSirVerification":
            return {
              dataKey: "vivekSirVerification",
              header: "Vivek Sir Verification",
            };
          case "girijaVerification":
            return {
              dataKey: "girijaVerification",
              header: "Girija Mam Verification",
            };
          case "firstVerification":
            return { dataKey: "firstVerification", header: "1st Verification" };
          case "secondVerification":
            return {
              dataKey: "secondVerification",
              header: "2nd Verification",
            };
          case "hariSirVerification":
            return {
              dataKey: "hariSirVerification",
              header: "Hari sir Verification",
            };
          case "issuedForPartnerProprietorVerification":
            return {
              dataKey: "issuedForPartnerProprietorVerification",
              header: "Issued for Partner/Proprietor Verification",
            };
          case "challanPreparation":
            return {
              dataKey: "challanPreparation",
              header: "Challan Preparation",
            };
          case "itrFiledOn":
            return { dataKey: "itrFiledOn", header: "ITR Filed on" };
          case "billPreparation":
            return { dataKey: "billPreparation", header: "Bill preparation" };
          default:
            return null;
        }
      })
  ...
  const handleITRDownloadExcel = (
      ...
      selectedColumns.forEach((colId) => {
        ...
        switch (colId) {
          case "no":
            ...
            break;
          case "dataReceivedOn":
            row["Data Received on"] = formatDate(task.inwardEntryDate);
            break;
          case "nameOfAssessee":
            row["Name of the Assessee"] = task.clientName;
            break;
          case "teamHead":
            row["Team Head"] = teamHeadNames;
            break;
          case "allotee":
            row["Allotee"] = task.assignedTo
              ? `${task.assignedTo.firstName} ${task.assignedTo.lastName}`
              : "";
            break;
          case "draftFinancialsAndComputationPreparation":
            row["Draft Financials and Computation Preparation"] =
              getStatusValue(
                task.itrProgress?.draftFinancialsAndComputationPreparation,
              );
            break;
          case "accountantVerification":
            row["Accountant Verification"] = getStatusValue(
              task.itrProgress?.accountantVerification,
            );
            break;
          case "vivekSirVerification":
            row["Vivek Sir Verification"] = getStatusValue(
              task.itrProgress?.vivekSirVerification,
            );
            break;
          case "girijaVerification":
            row["Girija Mam Verification"] = getStatusValue(
              task.itrProgress?.girijaVerification,
            );
            break;
          case "firstVerification":
            row["1st Verification"] = firstVerifier;
            break;
          case "secondVerification":
            row["2nd Verification"] = secondVerifier;
            break;
          case "hariSirVerification":
            row["Hari sir Verification"] = getStatusValue(
              task.itrProgress?.hariSirVerification,
            );
            break;
          case "issuedForPartnerProprietorVerification":
            row["Issued for Partner/Proprietor Verification"] = getStatusValue(
              task.itrProgress?.issuedForPartnerProprietorVerification,
            );
            break;
          case "challanPreparation":
            row["Challan Preparation"] = getStatusValue(
              task.itrProgress?.challanPreparation,
            );
            break;
          case "itrFiledOn":
            row["ITR Filed on"] = task.itrProgress?.itrFiledOn
              ? formatDate(task.itrProgress.itrFiledOn)
              : "";
            break;
          case "billPreparation":
            row["Bill preparation"] = getStatusValue(
              task.itrProgress?.billPreparation,
            );
            break;
          default:
            break;
    ...
    ws["!cols"] = colNames.map((name) =>
      name === "Name of the Assessee" ? { wch: 35 } : { wch: 15 },
    );
    ws["!rows"] = Array(itrData.length + 6).fill({ hpt: 40 });

    // Add bold, dark title row above the table
    const title =
      "STAUTS OF INDIVIDUAL INCOME TAX RETURNS FILING FOR AY 2025-26";
    ws["A3"] = {
      t: "s",
      v: title,
      s: {
          ...
          if (cell.v === "✓" || cell.v === "✗") {
            cell.s = {
              font: {
                bold: true,
                sz: 18,
                color: { rgb: cell.v === "✓" ? "008000" : "FF0000" },
              },
              alignment: {
                horizontal: "center",
                vertical: "center",
  ...
  const dashboardCounts = useMemo(() => {
    if (showCompletedTasks) {
      ...
      return {
        yetToStartCount: tasks.filter((t) => t.status === "yet_to_start")
          .length,
        todayCount: tasks.filter(
          (t) => t.priority === "today" && t.status !== "completed",
        ).length,
  ...
  const { yetToStartCount, todayCount, totalCount, urgentCount } =
    dashboardCounts;
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-green-800 font-medium">
                Currently viewing completed tasks
              </span>
            </div>
            <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div
          className="bg-white rounded-lg shadow-md p-3 sm:p-4 cursor-pointer hover:bg-gray-50 transition-colors"
          onClick={handleTotalTasksClick}
        >
          <div className="flex items-center">
        ...
        <div className="flex flex-col w-full gap-2 sm:hidden">
            ...
            <div className="relative flex-1">
              ...
              {showColumnDropdown && (
                <div
                  ...
                  className="absolute right-0 z-20 bg-white border border-gray-200 rounded-lg shadow-lg p-3 mt-2 w-56"
                >
            ...
            <div className="relative flex-1 flex items-center">
              ...
              {showGroupByDropdown && (
                <div
                  ...
                  className="absolute left-0 top-full z-20 bg-white border border-gray-200 rounded-lg shadow-lg mt-2 w-44"
                ...
                >
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${!activeTabObj.sortBy || activeTabObj.sortBy === "" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    None
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "createdAt" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "priority" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "priority" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Priority
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "status" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "status" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Stages
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "clientName" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "clientName" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Client Name
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "clientGroup" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "clientGroup" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Client Group
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "workType" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "workType" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Work Type
                  ...
                  <button
                    className={`block w-full text-left px-4 py-2 rounded ${activeTabObj.sortBy === "billed" ? "bg-blue-100 text-blue-800 font-semibold" : "hover:bg-blue-50 text-gray-700"}`}
                    onClick={() => {
                      updateActiveTab({ sortBy: "billed" });
                      setShowGroupByDropdown(false);
                    }}
                  >
                    Internal Works
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M3 4h13M3 8h9m-9 4h6m4-4v8m0 0l-3-3m3 3l3-3"
                  />
                </svg>
                <span className="font-semibold">Sort</span>
                {activeTabObj.taskSort !== "none" && (
                  <svg
                    className={`h-4 w-4 text-blue-500 transform ${activeTabObj.taskSortOrder === "asc" ? "" : "rotate-180"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
              ...
              {showSortDropdown && (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      const newOrder =
                        ...
                        activeTabObj.taskSortOrder === "desc"
                          ? "asc"
                          : "desc";
                      updateActiveTab({
                        taskSort: "createdAt",
                  ...
                  >
                    ...
                    {activeTabObj.taskSort === "createdAt" && (
                      <svg
                        className={`h-4 w-4 transform ${activeTabObj.taskSortOrder === "asc" ? "" : "rotate-180"}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
          ...
          <div className="flex w-full gap-2">
            ...
            <button
              ...
              title={
                showCheckboxes ? "Hide Selection Mode" : "Enable Selection Mode"
              }
              type="button"
            >
              <svg
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="relative" ref={bulkStatusDropdownRef}>
              ...
              {showBulkStatusDropdown && !bulkStatusLoading && (
                <div className="absolute right-0 top-full z-20 bg-white border border-gray-200 rounded-lg shadow-lg mt-2 w-64 max-h-64 overflow-y-auto animate-fade-in">
                  <div className="font-semibold text-gray-700 mb-2 text-sm px-3 pt-3">
                    Update Status for {selectedTasks.length} Task
                  ...
                  {taskStatuses.map((status) => {
                    ...
                    const tasksWithoutSelfVerification =
                      selectedTaskObjects.filter(
                        (task) => !task.selfVerification,
                      );
                    const isDisabled =
                      isCompletedStatus &&
                      tasksWithoutSelfVerification.length > 0;
                    ...
                    return (
                      <div key={status._id} className="relative">
                        <button
                          ...
                          onClick={() =>
                            !isDisabled && handleBulkStatusUpdate(status.name)
                          }
                          disabled={isDisabled}
                        >
                          <span
            ...
            <div className="relative" ref={bulkPriorityDropdownRef}>
              ...
              {showBulkPriorityDropdown && !bulkPriorityLoading && (
                <div className="absolute right-0 top-full z-20 bg-white border border-gray-200 rounded-lg shadow-lg mt-2 w-64 max-h-64 overflow-y-auto animate-fade-in">
                  <div className="font-semibold text-gray-700 mb-2 text-sm px-3 pt-3">
                    Update Priority for {selectedTasks.length} Task
                    {selectedTasks.length > 1 ? "s" : ""}
                  </div>
                  {priorities.map((priority) => {
                    const formattedName = priority.name
                      .replace(/_/g, " ")
                      .split(" ")
                      .map(
                        (word) => word.charAt(0).toUpperCase() + word.slice(1),
                    ...
                    return (
                      <div key={priority._id} className="relative">
                        <button
                          ...
                          onClick={() =>
                            handleBulkPriorityUpdate(priority.name)
                          }
                        >
                          <span
      ...
      <div className="table-wrapper-no-scrollbar w-full" ref={tableRef}>
        <AdvancedTaskTable
          tasks={getFilteredAndSortedTasks(
            tasks,
            isFilterPopupOpen ? filterDraft : activeTabObj.filters,
          )}
          viewType="admin"
          taskType={null}
          ...
          onStatusChange={async (taskId, newStatus) => {
            ...
            if (showCompletedTasks && newStatus !== "completed") {
              toast.error(
                "Cannot change status of completed tasks. Switch to active tasks view to modify.",
              );
              return;
            }
            ...
            if (newStatus === "completed") {
              ...
              if (task && !task.selfVerification) {
                toast.error(
                  "Cannot set status to Completed. Self verification must be checked first.",
                );
                return;
              }
            }
          ...
          setColumnWidths={(widths) =>
            updateActiveTab({ columnWidths: widths })
          }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-2xl min-h-[320px] flex flex-col">
                ...
                <div className="flex items-center mt-2">
                  ...
                  {Array.isArray(selectedAutomation.taskTemplate) && (
                    <div className="text-xs px-2 py-1 bg-green-100 text-green-700 font-medium rounded-full ml-2">
                      {selectedAutomation.taskTemplate.length}{" "}
                      {selectedAutomation.taskTemplate.length === 1
                        ? "template"
                        : "templates"}
                    </div>
                  )}
            ...
            <div className="flex-1 mb-4">
              ...
              {Array.isArray(selectedAutomation.taskTemplate) &&
                selectedAutomation.taskTemplate.length > 0 && (
                  <div className="mb-3">
                    <input
                      type="text"
              ...
              selectedAutomation.taskTemplate.length > 0 ? (
                (() => {
                  ...
                  const filteredTemplates = selectedAutomation.taskTemplate
                    .filter(
                      (template) => template.verificationStatus !== "pending",
                    )
                    .filter(
                      (template) =>
                        !templateSearchTerm ||
                  ...
                  return filteredTemplates.length > 0 ? (
                    <ul
                      className="space-y-2 mt-2 max-h-[420px] overflow-y-auto pr-2 custom-scrollbar"
                      style={{ maxHeight: "420px" }}
                    >
                      {filteredTemplates.map((template, idx) => (
                        <li
                          key={idx}
                          className="py-3 px-4 flex items-center justify-between bg-yellow-50 border-l-4 border-yellow-400 rounded-md hover:bg-yellow-100 transition-colors shadow-sm"
                        >
                          <div className="flex-1">
                            <span className="font-medium text-gray-800">
                              {template.title || "Scheduled Task"}
                            </span>
                            <div className="text-xs text-gray-500 mt-1">
                          ...
                          <div className="flex items-center space-x-2">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                const originalIndex =
                                  selectedAutomation.taskTemplate.findIndex(
                                    (t) =>
                            ...
                            <button
                              ...
                              onClick={async (e) => {
                                ...
                                if (
                                  window.confirm(
                                    `Are you sure you want to delete this template: ${template.title}?`,
                                  )
                                ) {
                                  try {
                                    // Create a new array without this template
                                    const updatedTemplates =
                                      selectedAutomation.taskTemplate.filter(
                                        (_, i) => i !== idx,
                                      );
                ...
                <div className="bg-gray-50 rounded-lg p-6 text-center border border-gray-100 shadow-sm">
                  ...
                  <p className="text-sm text-gray-500 font-medium">
                    No task templates in this automation yet.
                  </p>
                  <p className="text-xs text-gray-400 mt-2">
                    Add a template using the button below.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 shadow-xl">
            ...
            <div className="mb-6">
              <p className="text-gray-700">
                ...
                selected task{selectedTasks.length > 1 ? "s" : ""}?
              </p>
            </div>
