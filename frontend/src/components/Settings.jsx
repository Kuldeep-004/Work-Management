...
const Settings = () => {
  ...
  const cleanupResultsRef = useRef(null);
  ...
  // Cleanup state
  const [invalidPrioritiesData, setInvalidPrioritiesData] = useState(null);
  const [loadingInvalidPriorities, setLoadingInvalidPriorities] =
    useState(false);
  const [fixingInvalidPriorities, setFixingInvalidPriorities] = useState(false);
  const [selectedDefaultPriority, setSelectedDefaultPriority] = useState("");
  ...
  const pendingOrInProgressTasks = tasks.filter(
    (t) => t.status === "yet_to_start" || t.status === "in_progress",
  ).length;
  const urgentTasks = tasks.filter(
    (t) =>
      (t.status === "yet_to_start" || t.status === "in_progress") &&
      t.priority === "urgent",
  ).length;
  ...
  const handlePhotoChange = async (e) => {
    ...
    if (file.size > MAX_SIZE) {
      toast.error(
        `Image is too large (${(file.size / 1024 / 1024).toFixed(2)}MB). Maximum size is 20MB`,
      );
      return;
    }
  ...
  const deletePriority = async (priorityId) => {
    try {
      ...
      const checkResponse = await fetch(
        `${API_BASE_URL}/api/priorities/check-usage/${priorityId}`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
      ...
      if (!checkResponse.ok) {
        toast.error("Failed to check priority usage");
        return;
      }

      const usage = await checkResponse.json();

      if (usage.tasksCount > 0 || usage.automationsCount > 0) {
        const message =
          `Cannot delete "${priority.name}". It is currently used in:\n` +
          (usage.tasksCount > 0 ? `- ${usage.tasksCount} task(s)\n` : "") +
          (usage.automationsCount > 0
            ? `- ${usage.automationsCount} automation(s)\n`
            : "") +
          `\nPlease reassign these items before deleting this priority.`;

        if (!confirm(message)) return;
        toast.error("Priority is in use. Please reassign items first.");
        return;
      }
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/priorities/${priorityId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${user.token}` },
        },
  ...
  const updatePriority = async (priorityId, newData) => {
    ...
    try {
      ...
      if (newData.name.trim() !== priority.name) {
        const checkResponse = await fetch(
          `${API_BASE_URL}/api/priorities/check-usage/${priorityId}`,
          {
            headers: { Authorization: `Bearer ${user.token}` },
        ...
        if (usage.tasksCount > 0 || usage.automationsCount > 0) {
          const message =
            `Changing the name of "${priority.name}" will affect:\n` +
            (usage.tasksCount > 0 ? `- ${usage.tasksCount} task(s)\n` : "") +
            (usage.automationsCount > 0
              ? `- ${usage.automationsCount} automation(s)\n`
              : "") +
            `\nNote: The priority value in these items will NOT be automatically updated.\n` +
            `Consider using the "Cleanup Invalid Priorities" feature after this change.\n\n` +
  ...
  const handleEditPriority = (priority) => {
    ...
    setEditPriorityData({
      name: priority.name,
      color:
        priority.color || "bg-gray-100 text-gray-800 border border-gray-200",
    });
  };
  ...
  const handlePriorityDragEnd = async (result) => {
    ...
    const newPriorities = priorities.map((priority) => {
      const indexInFiltered = filteredPriorities.findIndex(
        (p) => p._id === priority._id,
      );
      if (indexInFiltered !== -1) {
        const newIndex = reorderedPriorities.findIndex(
          (p) => p._id === priority._id,
        );
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/priorities/bulk-update-order`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updatePriorityWithBulkCheck = async (priorityId, newData, oldName) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/bulk-update/execute-priority-update`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      toast.success(
        `Successfully updated ${result.updatedTasksCount} tasks and priority name`,
      );
  ...
  const updateStatusWithBulkCheck = async (statusId, newData, oldName) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/bulk-update/execute-status-update`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const result = await response.json();
      toast.success(
        `Successfully updated ${result.updatedTasksCount} tasks and status name`,
      );
  ...
  const checkInvalidPriorities = async () => {
    setLoadingInvalidPriorities(true);
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/cleanup/invalid-priorities`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
      if (!response.ok) {
        throw new Error("Failed to check for invalid priorities");
      }

      const data = await response.json();
      setInvalidPrioritiesData(data);

      if (data.tasks.length === 0 && data.automations.length === 0) {
        toast.success("No invalid priorities found!");
      } else {
        // Auto-scroll to results after a brief delay
        setTimeout(() => {
          if (cleanupResultsRef.current) {
            cleanupResultsRef.current.scrollIntoView({
              behavior: "smooth",
              block: "nearest",
            });
          }
      ...
      setLoadingInvalidPriorities(false);
    }
  };

  // Fix invalid priorities
  const fixInvalidPriorities = async () => {
    if (!selectedDefaultPriority) {
      toast.error("Please select a default priority");
      return;
    }

    if (
      !confirm(
        `This will update ${invalidPrioritiesData.tasks.length} tasks and ${invalidPrioritiesData.automations.length} automations to use "${selectedDefaultPriority}" priority. Continue?`,
      )
    ) {
      return;
    }

    setFixingInvalidPriorities(true);
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/cleanup/fix-invalid-priorities`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({ defaultPriority: selectedDefaultPriority }),
        },
      );

      if (!response.ok) {
        throw new Error("Failed to fix invalid priorities");
      }
      ...
      setFixingInvalidPriorities(false);
    }
  };
  ...
  const deleteTaskStatus = async (statusId, statusName) => {
    if (
      !confirm(
        `Are you sure you want to delete the "${statusName}" status? This action cannot be undone.`,
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/task-statuses/${statusId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${user.token}`,
  ...
  const handleStatusDragEnd = async (result) => {
    ...
    const filteredStatuses = taskStatuses.filter(
      (status) =>
        statusSearchTerm === "" ||
    ...
    const newStatuses = taskStatuses.map((status) => {
      const indexInFiltered = filteredStatuses.findIndex(
        (s) => s._id === status._id,
      );
      if (indexInFiltered !== -1) {
        const newIndex = reorderedStatuses.findIndex(
          (s) => s._id === status._id,
        );
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/task-statuses/bulk-update-order`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const updateWorkType = async (workTypeId, newName) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/work-types/${workTypeId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteWorkType = async (workTypeId, workTypeName) => {
    ...
    try {
      const checkResponse = await fetch(
        `${API_BASE_URL}/api/work-types/${workTypeId}/can-delete`,
        {
          headers: { Authorization: `Bearer ${user.token}` },
        },
      );
      ...
      if (!checkData.canDelete) {
        toast.error(
          `Cannot delete "${workTypeName}". It is being used in ${checkData.tasksCount} task(s).`,
        );
        return;
      }
      ...
      if (
        !confirm(
          `Are you sure you want to delete the "${workTypeName}" work type?`,
        )
      ) {
        return;
      }
  ...
  const handleUserTaskExport = async (userId, userName) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/backup/tasks/${userId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${user.token}`,
      ...
      const excelData = tasks.map((task) => ({
        ...
        Guides:
          task.guides
            ...
            .join(", ") || "",
        "Inward Entry Date": task.inwardEntryDate
  ...
  const tabs = [
    ...
    ...(user.role === "Admin" || user.role === "Team Head"
      ? ["Priority Management", "Stages", "Works"]
      : []),
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-[#485bbd] px-4 sm:px-8 py-4 flex flex-col md:flex-row items-center gap-4 md:gap-6 relative">
              ...
              >
                {loadingUsers ? (
                  ...
                  <>
                    ...
                    Backup Excel
                    <svg
                      className={`h-4 w-4 transition-transform duration-200 ${showBackupDropdown ? "rotate-180" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                        ...
                        <>
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              Tasks, Priorities, Stages & Work Types
                            </div>
                          </div>
                        </>
                      )}
        ...
        <div className="flex flex-col md:flex-row items-center w-full md:w-auto gap-4">
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <span className="text-white text-sm md:text-[14px] mt-1">
              {user.email}
            </span>
          ...
          <div className="flex flex-col items-center md:flex-row md:gap-16">
            ...
            <div className="flex flex-col items-center">
              <span className="text-2xl md:text-4xl text-white">
                {loadingStats ? "-" : totalTasks}
              </span>
              <span className="text-xs uppercase font-bold text-gray-300 tracking-wide mt-1 text-center">
                Total Tasks
              </span>
            </div>
            ...
            <div className="flex flex-col items-center">
              <span className="text-2xl md:text-4xl text-white">
                {loadingStats ? "-" : pendingOrInProgressTasks}
              </span>
              <span className="text-xs uppercase font-bold text-gray-300 tracking-wide mt-1 text-center">
                Pending Tasks
              </span>
      ...
      <div className="max-w-4xl bg-white rounded-lg p-4 md:p-0 mx-auto">
        {activeTab === "Account settings" && (
          <>
            <h2 className="text-lg font-semibold px-0 md:px-6 py-4">
              Account settings
            </h2>
            <form onSubmit={handleSubmit}>
              <div className="divide-y divide-gray-200">
                <div className="flex flex-col md:flex-row md:items-center px-0 md:px-6 py-3">
                  <div className="w-full md:w-1/3 text-gray-600 font-medium mb-2 md:mb-0">
                    First Name
                  </div>
        ...
        {activeTab === "Priority Management" &&
          (user.role === "Admin" || user.role === "Team Head") && (
            <>
              ...
              <div className="px-0 md:px-6 py-4 border-b border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <option value="bg-red-100 text-red-800 border border-red-200">
                        Red (Urgent)
                      </option>
                      <option value="bg-orange-100 text-orange-800 border border-orange-200">
                        Orange (High)
                      </option>
                      <option value="bg-yellow-100 text-yellow-800 border border-yellow-200">
                        Yellow (Medium-High)
                      </option>
                      <option value="bg-blue-100 text-blue-800 border border-blue-200">
                        Blue (Medium)
                      </option>
                      <option value="bg-indigo-100 text-indigo-800 border border-indigo-200">
                        Indigo (Normal)
                      </option>
                      <option value="bg-gray-100 text-gray-800 border border-gray-200">
                        Gray (Default)
                      </option>
                      <option value="bg-purple-100 text-purple-800 border border-purple-200">
                        Purple (Low)
                      </option>
                      <option value="bg-teal-100 text-teal-800 border border-teal-200">
                        Teal (Very Low)
                      </option>
                      <option value="bg-slate-100 text-slate-600 border border-slate-200">
                        Slate (Lowest)
                      </option>
                      <option value="bg-green-100 text-green-800 border border-green-200">
                        Green
                      </option>
                      <option value="bg-pink-100 text-pink-800 border border-pink-200">
                        Pink
                      </option>
                      <option value="bg-cyan-100 text-cyan-800 border border-cyan-200">
                        Cyan
                      </option>
                    </select>
                    <div
                      className={`w-12 h-10 rounded-md ${newPriorityData.color} flex items-center justify-center`}
                    >
                                        ...
                                        {editingPriority === priority._id ? (
                                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                            ...
                                            <div className="flex gap-2">
                                              ...
                                              <div
                                                className={`w-12 h-10 rounded-md ${editPriorityData.color} flex items-center justify-center`}
                                              >
                                          ...
                                          <div className="flex items-center gap-3">
                                            <span
                                              className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${priority.color || "bg-gray-100 text-gray-800 border border-gray-200"}`}
                                            >
                                              {priority.name}
                                            </span>
                                            <div className="flex items-center gap-2">
                                              <span className="text-sm text-gray-600">
                                                Order: {priority.order || "N/A"}
                                              </span>
                                            </div>
                                          </div>
                                        )}
              ...
              {user.role === "Admin" && (
                <div className="px-0 md:px-6 py-4 border-t border-gray-200 bg-yellow-50">
                  <h3 className="text-md font-medium text-gray-800 mb-3">
                    🔧 Cleanup Invalid Priorities
                  </h3>
                  <p className="text-sm text-gray-600 mb-4">
                    Check for tasks and automations that reference deleted
                    priorities and fix them.
                  </p>

                  <div className="flex gap-3 mb-4">
                    <button
                      onClick={checkInvalidPriorities}
                      disabled={loadingInvalidPriorities}
                    ...
                    >
                      {loadingInvalidPriorities
                        ? "Checking..."
                        : "Check for Invalid Priorities"}
                    </button>
                  </div>

                  {invalidPrioritiesData && (
                    <div
                      ref={cleanupResultsRef}
                    ...
                    >
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                          <li>
                            ...
                            <span className="font-bold text-red-600">
                              {invalidPrioritiesData.tasks.length}
                            </span>
                          </li>
                          <li>
                      ...
                      {/* Show Task Details */}
                      {invalidPrioritiesData.tasks.length > 0 && (
                          ...
                          <div className="max-h-60 overflow-y-auto border rounded-lg">
                            <table className="min-w-full divide-y divide-gray-200">
                              ...
                              <tbody className="bg-white divide-y divide-gray-200">
                                {invalidPrioritiesData.tasks.map((task) => (
                                  <tr
                                    key={task._id}
                      ...
                      {invalidPrioritiesData.invalidAutomationTemplates &&
                        invalidPrioritiesData.invalidAutomationTemplates
                          ...
                          <div className="mb-4 border-t pt-4">
                            <h5 className="font-semibold text-gray-800 mb-2">
                              Automation Templates with Invalid Priorities:
                            </h5>
                            <div className="max-h-60 overflow-y-auto border rounded-lg">
                              <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50 sticky top-0">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                  {invalidPrioritiesData.invalidAutomationTemplates.map(
                                    (template, idx) => (
                                      <tr
                                        key={idx}
                                        className="hover:bg-gray-50"
                      ...
                      {(invalidPrioritiesData.tasks.length > 0 ||
                        invalidPrioritiesData.automations.length > 0) && (
                        <div className="mt-4 space-y-3">
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                              Select default priority to assign:
                            </label>
                            <select
                              value={selectedDefaultPriority}
                              onChange={(e) =>
                                setSelectedDefaultPriority(e.target.value)
                              }
                              className="w-full md:w-64 bg-white border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                            >
                          ...
                          <button
                            onClick={fixInvalidPriorities}
                            disabled={
                              fixingInvalidPriorities ||
                    ...
                    {workTypes
                      .filter(
                        (workType) =>
                          ...
                          workType.name
                            .toLowerCase()
                            .includes(workTypeSearchTerm.toLowerCase()),
                      )
                      .map((workType) => (
                        <div
                          key={workType._id}
                        ...
                        >
                            ...
                            {editingWorkType === workType._id ? (
                              <div className="flex gap-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditWorkTypeName(e.target.value)
                                  }
                                  className="flex-1 bg-white border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                                  autoFocus
                                />
                                <button
                                  ...
                                  disabled={
                                    updatingWorkType || !editWorkTypeName.trim()
                                  }
                                  className="bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700 transition-colors disabled:bg-gray-400"
                                >
                          ...
                          {editingWorkType !== workType._id && (
                            <div className="flex space-x-2">
                              ...
                              <button
                                onClick={() =>
                                  deleteWorkType(workType._id, workType.name)
                                }
                                className="text-red-600 hover:text-red-800 p-2 hover:bg-red-50 rounded-full transition-colors"
