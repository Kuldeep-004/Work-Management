...
const TaskList = ({
  ...
  const [editingVerificationStatus, setEditingVerificationStatus] =
    useState(null);
  ...
  const setVisibleColumns =
    setExternalVisibleColumns || setInternalVisibleColumns;
  ...
  const handleRequestVerification = async (taskId) => {
    try {
      ...
      toast.success(
        `${verificationStage === "first" ? "First" : "Second"} verification requested successfully`,
      );
  ...
  const handleVerificationUpdate = async (
    ...
    try {
      const body = verifierId
        ? { verificationStatus: status, verifierId }
        : { verificationStatus: status };
      const response = await fetch(
        `${API_BASE_URL}/api/tasks/${taskId}/verification`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const shouldDisableActions = (task) => {
    ...
    if (
      viewType === "received" &&
      (taskType === "verification" || taskType === "completed")
    ) {
      return true;
    }
  ...
  const handleVerifierAssignment = async (
    taskId,
    verifierId,
    isSecondVerifier = false,
  ) => {
    if (!verifierId) {
      toast.error("Please select a verifier");
      return;
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/tasks/${taskId}/verification`,
        {
          ...
          body: JSON.stringify({
            verificationStatus: isSecondVerifier
              ? "first_verified"
              : "executed",
      ...
      toast.success(
        `Task assigned for ${isSecondVerifier ? "second" : "first"} verification successfully`,
      );
    } catch (error) {
      console.error("Error updating verification status:", error);
      toast.error(error.message);
  ...
  const getAllowedVerificationStatuses = (task, loggedInUser) => {
    ...
    if (
      ...
      !statuses.includes(task.verificationStatus)
    ) {
      ...
      <div className="overflow-x-auto w-full">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {ALL_COLUMNS.filter((col) => visibleColumns.includes(col.id)).map(
                (col) => (
                  <th
                    key={col.id}
                  ...
                  switch (col.id) {
                    ...
                    case "description":
                      return (
                        <td
                          key={col.id}
                          className="px-4 py-4 sm:px-6 whitespace-nowrap text-sm text-gray-500 border border-gray-300 bg-white w-[180px] min-w-[180px]"
                    ...
                    case "workType":
                      return (
                        <td key={col.id} className="px-4 py-4 sm:px-6">
                    ...
                    case "billed":
                      return (
                        <td
                          key={col.id}
                          className="px-4 py-4 sm:px-6 whitespace-nowrap text-center text-lg"
                        >
                          {task.billed ? "Yes" : "No"}
                        </td>
                      );
                    case "status":
                      return (
                        <td
                          key={col.id}
                          className="px-4 py-4 sm:px-6 whitespace-nowrap text-sm text-gray-500"
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            {task.assignedBy ? (
                              <>
                                <img
                                  src={
                                    task.assignedBy.photo?.url || defaultProfile
                                  }
                          ...
                          <div className="flex items-center">
                            {task.assignedTo ? (
                              <>
                                <img
                                  src={
                                    task.assignedTo.photo?.url || defaultProfile
                                  }
                          ...
                          <div className="flex items-center space-x-2">
                            {task.verificationAssignedTo ? (
                              <>
                                <img
                                  src={
                                    task.verificationAssignedTo.photo?.url ||
                                    defaultProfile
                                  }
                          ...
                          <div className="flex items-center space-x-2">
                            {task.secondVerificationAssignedTo ? (
                              <>
                                <img
                                  src={
                                    task.secondVerificationAssignedTo.photo
                                      ?.url || defaultProfile
                                  }
                                  alt={`${task.secondVerificationAssignedTo.firstName} ${task.secondVerificationAssignedTo.lastName}`}
                                  className="h-8 w-8 rounded-full object-cover"
                                  onError={(e) => {
                                    e.target.onerror = null;
                                    e.target.src = defaultProfile;
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                onClick={() =>
                  handleRequestVerification(selectedTaskForVerification._id)
                }
        ...
        <div className="fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-4">
              ...
              <button
                onClick={() =>
                  handleVerifierAssignment(
                    selectedTaskForVerifier._id,
                    selectedTaskForVerifier.verificationAssignedTo,
                  )
                }
                className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700"
              >
