...
const ReceivedTasks = () => {
  ...
  const activeTabObj =
    ...
    tabs[0] ||
    DEFAULT_TAB("execution", customColumnsLoaded ? customColumns : []);
  ...
  const searchAndHighlightTask = async (taskId) => {
    ...
    try {
      ...
      for (let tabIdx = 0; tabIdx < tabs.length; tabIdx++) {
        ...
        for (const tabType of receivedTabTypes) {
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (simulatedTab.searchTerm) {
                tabTasks = tabTasks.filter((task) =>
                  (task.title || "")
                    .toLowerCase()
                    .includes(simulatedTab.searchTerm.toLowerCase()),
                );
              }
              if (
                simulatedTab.statusFilter &&
                simulatedTab.statusFilter !== "all"
              ) {
                tabTasks = tabTasks.filter(
                  (task) => task.status === simulatedTab.statusFilter,
                );
              }
            ...
            console.error(
              `Error searching in tab ${tab.title} (${tabType}):`,
              error,
            );
          }
        }
      ...
      if (!found) {
        ...
        toast.info(
          `Task "${taskDetails.title}" is not visible in any of your tabs/pages with the current filters. Try adjusting your filters or tabs.`,
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (
      ...
      toast.success(
        `Task found and highlighted in "${pendingHighlight.tabTitle}" (${pendingHighlight.tabType}) tab`,
      );
  ...
  const getFilteredAndSortedTasks = (tasks) => {
    ...
    let filteredTasks = tasks.filter((task) => {
      ...
      if (activeTabObj.activeTab === "guidance" && task.status === "completed")
        return false;
      ...
      if (activeTabObj.activeTab === "completed") {
        ...
        const isAssigned =
          task.assignedTo &&
          (task.assignedTo._id === targetUserId ||
            task.assignedTo === targetUserId);
        const isVerifier =
          (task.verificationAssignedTo &&
            (task.verificationAssignedTo._id === targetUserId ||
              task.verificationAssignedTo === targetUserId)) ||
          (task.secondVerificationAssignedTo &&
            (task.secondVerificationAssignedTo._id === targetUserId ||
              task.secondVerificationAssignedTo === targetUserId)) ||
          (task.thirdVerificationAssignedTo &&
            (task.thirdVerificationAssignedTo._id === targetUserId ||
              task.thirdVerificationAssignedTo === targetUserId)) ||
          (task.fourthVerificationAssignedTo &&
            (task.fourthVerificationAssignedTo._id === targetUserId ||
              task.fourthVerificationAssignedTo === targetUserId)) ||
          (task.fifthVerificationAssignedTo &&
            (task.fifthVerificationAssignedTo._id === targetUserId ||
              task.fifthVerificationAssignedTo === targetUserId));
      ...
      if (activeTabObj.activeTab === "receivedVerification") {
        ...
        for (let i = verifierFields.length - 1; i >= 0; i--) {
          if (
            task[verifierFields[i]] &&
            (task[verifierFields[i]]._id || task[verifierFields[i]])
          ) {
            latestVerifier =
              task[verifierFields[i]]._id || task[verifierFields[i]];
            break;
        ...
        if (!latestVerifier || String(latestVerifier) !== String(targetUserId))
          return false;
      }
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    ...
    try {
      console.log(
        "Starting status update for task:",
        taskId,
        "new status:",
        newStatus,
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          throw new Error(
            "You can only delete tasks that you created or are assigned to",
          );
        }
        ...
        if (isMounted && tabStates && Array.isArray(tabStates.tabs)) {
          ...
          const migratedTabs = tabStates.tabs.map((tab) => {
            ...
            if (!updatedTab.visibleColumns.includes("verification")) {
              ...
              const priorityIndex = updatedTab.visibleColumns.findIndex(
                (colId) => colId === "priority",
              );
              if (priorityIndex !== -1) {
                updatedTab.visibleColumns.splice(
                  priorityIndex + 1,
                  0,
                  "verification",
                );
            ...
            if (!updatedTab.columnOrder.includes("verification")) {
              const priorityIndex = updatedTab.columnOrder.findIndex(
                (colId) => colId === "priority",
              );
              if (priorityIndex !== -1) {
                updatedTab.columnOrder.splice(
                  priorityIndex + 1,
                  0,
                  "verification",
                );
  ...
  const isPageLoading =
    !tabsLoaded || !usersLoaded || (!tasksLoaded && !initialDataLoaded);
  ...
  return (
    <div className="p-4">
          ...
          >
            {[
              ...
              >
                ...
                {tabKey === "receivedVerification" &&
                  "Task Received For Verification"}
                {tabKey === "issuedVerification" &&
            ...
            <div className="relative flex items-center gap-2">
                  ...
                  {activeTabObj.selectedUserId
                    ? (() => {
                        const selectedUser = users.find(
                          (u) => u._id === activeTabObj.selectedUserId,
                        );
                        return selectedUser
              ...
              {showUsersDropdown && (
                <div
                  ...
                  className="absolute left-0 top-full z-20 bg-white border border-gray-200 rounded-lg shadow-lg mt-2 w-48 animate-fade-in max-h-64 overflow-y-auto"
                >
                  <div className="font-semibold text-gray-700 mb-2 text-sm px-3 pt-3">
                    View Tasks For
                  </div>
      ...
      <ErrorBoundary>
        ...
        <div
          className="table-wrapper-no-scrollbar w-full relative bg-gray-50"
          ref={tableRef}
        >
